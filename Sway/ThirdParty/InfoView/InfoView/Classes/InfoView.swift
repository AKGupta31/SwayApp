...
@objc public protocol InfoViewDelegate {
    @objc optional func infoViewWillShow(view: InfoView)
    @objc func infoViewDidShow(view: InfoView)
    @objc func infoViewWillHide(view: InfoView)
    @objc func infoViewDidHide(view: InfoView)
}
...
public class InfoView: UIView {
    ...
    override public var autoresizingMask: UIView.AutoresizingMask {
        didSet {
    ...
    private func animateAppearance() {
        delegate?.infoViewWillShow?(view: self)

        switch self.animation {
        case .FadeInAndScale:
            placeholderView.transform = scaleTransform(t: placeholderView.transform)
            label.transform = scaleTransform(t: label.transform)
        default:
            break
        }
        ...
        UIView.animate(withDuration: duration(), animations: {
            ...
            self.delegate?.infoViewDidShow(view: self)
        })
    }

    private func animateDisappearance() {
        self.delegate?.infoViewWillHide(view:self)
        ...
        UIView.animate(withDuration: duration(), animations: {
            ...
            case .FadeIn, .None:
                break
            case .FadeInAndScale:
                self.placeholderView.transform = self.scaleTransform(t: self.placeholderView.transform)
                self.label.transform = self.scaleTransform(t: self.label.transform)
            }
        }, completion: { (complete) in
            self.backgroundView.removeFromSuperview()
            self.delegate?.infoViewDidHide(view:self)
        })
    }
